...
function vergleicheVerkehrsmittel(wegeIDVglWegepaare) {
    ...
    wegeIDVglWegepaare.forEach((element, key,  wegeIDVglWegepaare) => {
        ...
        leseFile(wegeID1, wegeID2, vmVgl, wS1, wS2, durchlaufeneFiles, filesVM, anzahlFiles, key);
...
function leseFile(wegeID1, wegeID2, vmVgl, wS1, wS2, durchlaufeneFiles, filesVM, anzahlFiles, key) {

    // read selected JSON Files
    for(file of filesVM) {
        let readerVM = new FileReader();
        readerVM.addEventListener('loadend',handleEvent);
        readerVM.onload = function (f) {
